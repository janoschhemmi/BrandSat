...
#ds = gdal.Open("V:/BrandSat/01_Data/01_FGK/01_Rasterization/rasterized_15m_dif_10_res_split_into_dense_and_age.tif")
ds = gdal.Open("V:/BrandSat/01_Data/01_FGK/01_Rasterization/rasterized_20m_dif_20_res_split_into_dense_and_age.tif")
...
## --> Workaround for split ts.. here load old ts raster--> get old ts without split
##--> ansich gute idee geht aber nicht weil jetzt auch nicht so dense tree stands drin sind
# ds_genus = gdal.Open("V:/Data_Forst_Sachsen/04_Rasterized_ForstGrundKarte/Saxony_genus.tif")
print("Read Genus...")
ds_genus = gdal.Open("V:/BrandSat/01_Data/01_FGK/01_Rasterization/Brandenburg_15m_18ts_bereinigt.tif")
ds_sp_genus = ds.GetProjection()
...
# Offset between Rasterized FGK and FoRCE Tiles:
UL_x = gt_band1[0] + 30000  ## in case of smaller raster extent, in this case one tile each
UL_y = gt_band1[3] - 30000  ## same

lrx = UL_x + (ds_grid.RasterXSize * spatial_resolution)
lry = UL_y - (ds_grid.RasterYSize * spatial_resolution)

### set number of samples and tree species to sample in
np.set_printoptions(suppress=True,
   formatter={'float_kind':'{:16.3f}'.format}, linewidth=130)

# get unique values of array
try_unique = np.unique(myarray)
try_unique = try_unique[try_unique > 0] # filter NA
try_unique = try_unique[(try_unique > 100) | (try_unique < 20)] # filter tree species

### get number of tree species
number_of_ts = len(try_unique)
...
treespeciec_int_strat = try_unique.astype(int)
...
def rand_sample_array_stratified(in_arr, in_arr_age, in_arr_genus, n_samples, strata, min_dis, raster_cell_size,
                                 return_values):
    ''' in_arr    : array
    ...
    for i, stratum in enumerate(strata):
        ...
        strat_cols, strat_rows = np.where(
            np.logical_and(in_arr >= treespeciec_int_strat[i], in_arr <= treespeciec_int_strat[i]))
        # print(strat_rows)
        # print(strat_cols)
        ...
        while (len(random_draws_out) < n_samples[
            i]) and lo < 100:  ## set number of iteration, means tries to get samples
            ...
            for k, random_draw in enumerate(one_draw_coordinates):
                ...
                for random_draw_check in one_draw_coordinates:
                    ...
                    if math.sqrt((random_draw[0] - random_draw_check[0]) ** 2 + (
                            random_draw[1] - random_draw_check[1]) ** 2) < (
                            min_dis / raster_cell_size):

                        if math.sqrt((random_draw[0] - random_draw_check[0]) ** 2 + (
                                random_draw[1] - random_draw_check[1]) ** 2) != 0:
                            ## exclude self check
                            # print("Distance:")
                ...
                if len(random_draws_out) > 0 and dis_not_ok == 0:
                    ...
                    for random_draw_check_2 in random_draws_out:
                        ...
                        # print(checked_draws)
                        if math.sqrt((random_draw[0] - random_draw_check_2[0]) ** 2 + (
                                random_draw[1] - random_draw_check_2[1]) ** 2) < (
                                min_dis / raster_cell_size):
                            throw_away_count = throw_away_count + 1
                            dis_not_ok = dis_not_ok + 1
...
test_sample, age_values, genus_values = rand_sample_array_stratified(myarray, myarray_age, myarray_genus,
                                                                     strata=treespeciec_int_strat,
                                                                     n_samples=list_of_n_samples, min_dis=100,
                                                                     raster_cell_size=spatial_resolution,
                                                                     return_values=True)
...
shapefile = driver.CreateDataSource(
    'V:/BrandSat/01_Data/02_Sampling/Sampling_Points_Brandenburg_20m_dif_20m_res_split_500st_100m_dis_2.shp')

### get spatial reference of test sample file from existing shapefile
dataset_proj = driver.Open(
    'V:/Processing/02_Sampling/02_15m_40st_Brandenburg/Sampling_Points_Brandenburg_15m_500st_40ts.shp')
...
layer.CreateField(ogr.FieldDefn("TreeS_int_old", ogr.OFTInteger))
...
for k in range(0, len(test_sample)):
    ...
    for i in range(0, len(strata)):
        ...
        print("t")
        ## set to float again ##keine ahnung warum er da nur floats nimmt
        num = (treespeciec_int_strat[k])
        num = num.astype(np.float)

        #feature.SetField('TreeS_int',ts_int_float)
        feature.SetField('TreeS_int', num)
        feature.SetField('Age_int', single_age)

        ## here set to 1 if it was in old aswell or set to zero if new due to lower density
        ## auch fuer vergleich wie gut ist es fuer die karte wenn die weniger dichten drin sind

        if single_genus < 0:
            old = 0
        else:
            old = 1
        feature.SetField('TreeS_int_old', old)
        # create wkt
        wkt = "POINT(%f %f)" % (
        (((float(single_point[0])) * spatial_resolution) + UL_x + (spatial_resolution/2)),
        ((UL_y - ((float(single_point[1])) * spatial_resolution) - (spatial_resolution/2))))
