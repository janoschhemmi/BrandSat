...
number_of_ts = 22
list_of_n_samples = [30000] * number_of_ts
...
shapefile = driver.CreateDataSource(
    'V:/BrandSat/01_Data/02_Sampling/Sampling_Points_Brandenburg_15m_30000_samp_size_20m_spatial_res_test.shp')
...
for k in range(0, len(test_sample)):
    ...
    for i in range(0, len(strata)):
        ...
        # create wkt
        wkt = "POINT(%f %f)" % (
        (((float(single_point[0])) * 20) + UL_x + 10), ((UL_y - ((float(single_point[1])) * 20) - 10)))
